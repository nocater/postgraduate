...
def ChiMerge(df, variable, flag, confidenceVal=3.841, bin=10, sample = None):
    ...
    while (1):
        if (len(chi_table) <= (bin - 1) and np.min(chi_table) >= confidenceVal):
            break
...
def deal_labels(y, categories, onehot=False):
    """
    将类别进行离散表示
    """
...
if __name__=='__main__':
    ...
    fe.preprocess()
    df = pd.DataFrame({'C':[1,2,3,4], 'Y':[0,0,1,1]})
    temp2 = ChiMerge(df, 'C', 'Y',
                     confidenceVal=5.841, bin=5, sample=None)